            ...
            {
                {
                    ...
                    if (!AudioPreken || !LeesPreken || !Lezingen)
                    {
                        ...
                        else if (Lezingen && LeesPreken) { parts.Add(Prekenweb.Resources.Resources.LeesprekenEnLezingen); }
                    ...
                    if (!string.IsNullOrWhiteSpace(Gebeurtenis)) parts.Add(string.Format("{0}: {1}", Prekenweb.Resources.Resources.Gebeurtenis, Gebeurtenis));
                    if (!string.IsNullOrWhiteSpace(Serie)) parts.Add(string.Format("{0}: {1}", Prekenweb.Resources.Resources.Serie, Serie));
                    if (SorteerOp.HasValue && SorteerOp.Value != Tables.SorteerOp.Datum) parts.Add(string.Format("gesorteerd op: {0}", SorteerOp.Value.ToString()));
